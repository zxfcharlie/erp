...
PRODUCT_LIST_PATH = "/v1/basic-data/b2b/products/getlist"
ORDER_INFO_PATH = "/v1/order/info/get"
FEE_DETAILS_PATH = "/v1/order/fee-details/get"
TRACK_FULL_PATH = "/v1/track-service/getAllForOPenApi"
TRACK_SUBSCRIBE_PATH = "/v1/track-service/subscribe-by-order"
LAST_MILE_PATH = "/v1/order/last-mile/get"
...
def _split_name(full_name: str):
    """
    云途要求 first_name / last_name 都不能为空。
    按空格切分成两段；切不出来(没有空格，如中文姓名或单个单词)时，
    两个字段都填整个姓名，保证 first_name/last_name 都非空。
    """
...
def _call_signed_api(provider, method: str, path: str, payload):
    """
    统一封装：获取 accessToken -> 计算签名 -> 发起请求
    payload=None 表示这次请求不带请求体(签名内容不含 body= 段)；
    payload={} 表示显式带一个空 JSON 对象作为请求体(签名内容包含 body={} )，
    云途"查询物流产品列表"接口在不传 country_code 时就是这种情况。
    """
    base_url = provider.base_url or BASE_URL_PROD
    access_token = get_access_token(provider)

    date_ms = str(int(time.time() * 1000))
    body_str = json.dumps(payload, ensure_ascii=False, separators=(",", ":")) if payload is not None else ""
...
def get_order_fee_details(provider, waybill_number: str):
    """
    获取运单实际扣费详情 GET /v1/order/fee-details/get
    返回: dict(success, total_amount, currency, charge_weight, actual_weight, fee_details, raw, error)
    """
    try:
        resp_json = _call_signed_api(provider, "GET", FEE_DETAILS_PATH, {"waybill_number": waybill_number})
        success = bool(resp_json.get("success"))
        result = resp_json.get("result") or {}
        return {
            "success": success,
        ...
        return {"success": False, "raw": "", "error": f"查询运费详情失败: {e}"}


def get_tracking_full(provider, order_number: str):
    """
    查询运单全程轨迹(不受订阅限制) GET /v1/track-service/getAllForOPenApi
    返回: dict(success, package_status, events=[...], delivered, latest_event_text, raw, error)
    track_node_code == "DELIVERED" 代表已签收(官方示例确认)。
    """
    try:
        resp_json = _call_signed_api(provider, "GET", TRACK_FULL_PATH, {"order_number": order_number})
        success = bool(resp_json.get("success"))
        ...
        latest_event_text = ""
        if result_list:
            track_info = (result_list[0] or {}).get("track_Info") or {}
            events = track_info.get("track_events") or []
            for ev in events:
                if (ev.get("track_node_code") or "").upper() == "DELIVERED":
                    delivered = True
            if events:
                # 事件通常按时间正序排列，取最后一条作为最新状态
                latest_event_text = events[-1].get("process_content") or ""
        return {
            "success": success,
            "delivered": delivered,
        ...
        return {"success": False, "delivered": False, "latest_event_text": "", "events": [], "raw": "", "error": f"查询轨迹失败: {e}"}


def subscribe_tracking(provider, waybill_numbers, subscribe_type: str = "A"):
    """按运单号订阅轨迹 POST /v1/track-service/subscribe-by-order（非必需，getAllForOpenApi 不受订阅限制也能查，这里保留以备需要更丰富轨迹时使用）"""
    try:
        payload = {"waybill_numbers": waybill_numbers, "subscribe_type": subscribe_type, "query_type": ["Y"]}
        resp_json = _call_signed_api(provider, "POST", TRACK_SUBSCRIBE_PATH, payload)
        return {"success": bool(resp_json.get("success")), "raw": json.dumps(resp_json, ensure_ascii=False)}
    except Exception as e:
        return {"success": False, "raw": "", "error": f"订阅轨迹失败: {e}"}


def get_product_list(provider, country_code: str = ""):
    """
    查询云途物流产品列表 GET /v1/basic-data/b2b/products/getlist
    country_code 为空时查询全部产品。
    返回: dict(success, products=[{code, name}], error)
    """
    try:
        payload = {"country_code": country_code} if country_code else {}
        resp_json = _call_signed_api(provider, "GET", PRODUCT_LIST_PATH, payload)
        success = bool(resp_json.get("success"))
        detail = resp_json.get("detail") or []
        products = [
            {"code": d.get("product_code", ""), "name": d.get("product_name", "")}
            for d in detail
        ]
        return {
            "success": success,
        ...
        return {"success": False, "products": [], "error": f"查询云途产品列表失败: {e}"}
